# 🔐 CREDENCIAIS DO SISTEMA SAFE2GO HELPDESK

## 🌐 Acesso ao Sistema
**URL:** https://readydata-access.preview.emergentagent.com

---

## 👨‍💼 ADMINISTRADORES
**Senha padrão para todos os admins:** `S@muka91`

| Nome | Email | Telefone |
|------|-------|----------|
| **Pedro Carvalho** | **pedrohcarvalho1997@gmail.com** | 11987654321 |
| Pedro Carvalho | pedro.carvalho@safe2go.com.br | 11987654321 |
| Maria Silva | maria.silva@safe2go.com.br | 11987654322 |
| João Santos | joao.santos@safe2go.com.br | 11987654323 |
| Ana Costa | ana.costa@safe2go.com.br | 11987654324 |

### Permissões dos Administradores:
✅ Ver todos os casos de todas as seguradoras  
✅ Criar, editar e excluir casos  
✅ Gerenciar usuários (criar, editar, aprovar)  
✅ Ver comentários internos  
✅ Exportar/Importar dados (JSON)  
✅ Gerar relatórios PDF  
✅ Acessar análise de casos recorrentes  
✅ Ver estatísticas completas no dashboard  

---

## 👤 CLIENTES
**Senha padrão para todos os clientes:** `senha123`

### AVLA
| Nome | Email | Telefone |
|------|-------|----------|
| Carlos Oliveira | carlos.oliveira@avla.com.br | 11987651111 |
| Juliana Lima | juliana.lima@avla.com.br | 11987651112 |
| Roberto Ferreira | roberto.ferreira@avla.com.br | 11987651113 |

### ESSOR
| Nome | Email | Telefone |
|------|-------|----------|
| Patrícia Souza | patricia.souza@essor.com.br | 11987652221 |
| Fernando Alves | fernando.alves@essor.com.br | 11987652222 |
| Luciana Martins | luciana.martins@essor.com.br | 11987652223 |

### DAYCOVAL
| Nome | Email | Telefone |
|------|-------|----------|
| Ricardo Pereira | ricardo.pereira@daycoval.com.br | 11987653331 |
| Camila Rodrigues | camila.rodrigues@daycoval.com.br | 11987653332 |
| Bruno Nascimento | bruno.nascimento@daycoval.com.br | 11987653333 |

### Permissões dos Clientes:
✅ Ver apenas casos que **eles próprios criaram** (privacidade e segurança)  
✅ Criar novos casos  
✅ Comentar em casos  
✅ Ver apenas comentários públicos  
✅ Receber notificações  
❌ Não podem ver casos de outros usuários (mesmo da mesma seguradora)  
❌ Não podem excluir casos  
❌ Não podem gerenciar usuários  
❌ Não veem comentários internos

> **📝 Nota Importante:** Por questões de privacidade e segurança, cada cliente vê apenas os casos que ele próprio abriu. Administradores têm acesso a todos os casos de todas as seguradoras.  

---

## 📊 DADOS DO SISTEMA

### Estatísticas Atuais:
- **Total de Usuários:** 14 (5 admins + 9 clientes)
- **Total de Casos:** 75
- **Casos Concluídos:** 45 (60%)
- **Casos Pendentes:** 9
- **Casos Em Desenvolvimento:** 8
- **Casos Aguardando Resposta:** 13

### Distribuição por Seguradora:
- **AVLA:** 25 casos
- **ESSOR:** 25 casos
- **DAYCOVAL:** 25 casos

### Dados Adicionais:
- **Comentários:** 78 (públicos e internos)
- **Notificações:** 43
- **Atividades Registradas:** 113

---

## 🚀 FUNCIONALIDADES DISPONÍVEIS

### Para Todos os Usuários:
- ✅ Login/Logout
- ✅ Dashboard com estatísticas
- ✅ Visualização de casos
- ✅ Sistema de comentários
- ✅ Notificações em tempo real
- ✅ Filtros por status e seguradora
- ✅ Busca de casos

### Apenas para Administradores:
- ✅ Gestão de usuários
- ✅ Edição de perfis
- ✅ Aprovação de novos usuários
- ✅ Exportar casos (JSON)
- ✅ Importar casos (JSON/OCR)
- ✅ Gerar relatórios PDF
- ✅ Ver comentários internos
- ✅ Análise de casos recorrentes
- ✅ Excluir casos
- ✅ Webhook do Jira (integração)

---

## 🔌 INTEGRAÇÕES

### Webhook Jira
**Endpoint:** `https://readydata-access.preview.emergentagent.com/api/webhooks/jira`  
**Método:** POST  
**Autenticação:** Não requerida  

Permite integração com Jira para criação automática de casos.

---

## 📝 NOTAS IMPORTANTES

1. **Primeiro Acesso:** Use as credenciais fornecidas acima
2. **Alteração de Senha:** Disponível nas configurações do perfil
3. **Recuperação de Senha:** Contate o administrador
4. **Suporte Técnico:** pedro.carvalho@safe2go.com.br
5. **Backup de Dados:** Sistema realiza backup automático diário

---

## 🛠️ SUPORTE TÉCNICO

Para questões técnicas ou problemas de acesso:
- **Email:** pedro.carvalho@safe2go.com.br
- **Sistema:** Safe2Go Helpdesk v1.0
- **Última Atualização:** 29/12/2025

---

**© 2025 Safe2Go - Sistema de Gerenciamento de Helpdesk**
